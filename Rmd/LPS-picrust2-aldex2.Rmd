...
```
```{r}
path_abund <- path_abund_unstrat %>% select(pathway, matches("[HW][wt][1-2].[36][abc]"))
row.names(path_abund) <- path_abund$pathway
path_abund <- path_abund %>% select(-pathway)
path_abund_round <- round(path_abund, digits = 0)
```
```{r}
sum(path_abund)
relative <- function(x) {round(((x / sum(x))*10000000), digits = 0)}
...
aldexplots
ggsave(aldexplots, filename = "./outputs/aldex2/lps-picrust2-aldex2.pdf", device = "pdf")
```
```{r}
cutoff <- 0.1
